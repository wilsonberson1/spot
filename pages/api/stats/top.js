...
export default async function handler(req, res) {
  ...
  const queryRange = Array.isArray(req.query.range)
    ? req.query.range[0]
    : req.query.range;
  const timeRange = ALLOWED_RANGES.includes(queryRange)
    ? queryRange
    : 'medium_term';
  ...
  try {
    ...
    res.status(200).json({ time_range: timeRange, artists, tracks });
  } catch (err) {
    console.error(err);
